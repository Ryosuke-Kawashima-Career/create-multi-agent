...
- `make setup`: run `scripts/setup.sh`, create `.env`, save the connpass ID in `scripts/.state`, and install dependencies.
...
- `make cleanup`: delete Agent Runtime resources whose display name starts with the saved participant ID prefix.
...
Do not commit real credentials. Use `scripts/setup.sh` to generate `.env` and `scripts/.state`. Treat `data/` as fictional fixtures; keep any new customer examples synthetic and run redaction tests when modifying privacy-related logic.
